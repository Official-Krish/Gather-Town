...
import { useSpaceStates } from "../../lib/AllStates";
import { AvatarDirection, UserPositionInfo } from "../../lib/types";
import { checkFacingUsers, determineDirection } from "../../lib/Func";
...
type PeerData = MediaStateChangeData | EndCallData;
...
const Space = () => {
  ...
  const activeConnectionsRef = useRef<Record<string, any>>({});
  ...
  useEffect(() => {
    ...
    let draggableDiv: HTMLDivElement | null = null;
    ...
    function handleTouchStart(e: React.TouchEvent) {
      if (e.touches.length === 1) {
        ...
        startDrag(mouseEvent as unknown as React.MouseEvent<Element, MouseEvent>);
      }
      e.preventDefault();
    }
    ...
    function handleTouchMove(e: React.TouchEvent) {
      if (e.touches.length === 1) {
        ...
        drag(mouseEvent as unknown as React.MouseEvent<Element, MouseEvent>);
      }
      e.preventDefault();
    }
    ...
    function setupDirectDraggable() {
      ...
      draggableDiv.addEventListener("mousedown", startDrag as unknown as EventListener);
      draggableDiv.addEventListener("touchstart", handleTouchStart as unknown as EventListener, {
        passive: false,
      });
    ...
    return () => {
      ...
      if (draggableDiv) {
        draggableDiv.removeEventListener("mousedown", startDrag as unknown as EventListener);
        draggableDiv.removeEventListener("touchstart", handleTouchStart as unknown as EventListener);
      }

      document.removeEventListener("mousemove", drag as unknown as EventListener);
  ...
  useEffect(() => {
    ...
    const fetchSpaceDetails = async () => {
      try {
        const response = await axios.get(`${BACKEND_URL}/spaces/${spaceId}`, {
          headers: {
            Authorization: `${token}`,
          },
        });
  ...
  const startCall = async (to: string) => {
    ...
    conn.on("data", (data: unknown) => {
      if (typeof data === 'object' && data !== null && 'type' in data) {
        ...
        if (peerData.type === "end-call") {
          endCall();
        } else if (peerData.type === "media-state-change") {
  ...
  }, [userPosition, userDirection, usersPositions, activeCall, user?.id]);
